...
lint: ## check style with flake8 and isort
	flake8 tgan tests
	isort -c --recursive tgan tests

.PHONY: fix-lint
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find tgan -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive tgan
	isort --apply --atomic --recursive tgan
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source tgan -m pytest
	coverage report -m
	coverage html
